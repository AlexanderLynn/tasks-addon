...
router.post('/', authenticateApiKey, validateBody(createItemInputSchema), async (req, res) => {
    try {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(itemId, input.listId, input.title, input.description || null, input.type, 'active', JSON.stringify(input.schedule), input.assignedTo || null, input.sharedWith ? JSON.stringify(input.sharedWith) : null, input.tags ? JSON.stringify(input.tags) : null, nextDueAt.toISOString(), now, now, 0);
        // Log audit
        await logAudit({
            entityType: 'item',
            entityId: itemId,
...
router.put('/:id', authenticateApiKey, validateBody(updateItemInputSchema), async (req, res) => {
    try {
        const { id } = req.params;
        const input = req.body;
        const db = await getDb();
...
router.post('/:id/complete', authenticateApiKey, validateBody(completeItemInputSchema), async (req, res) => {
    try {
        const { id } = req.params;
        const input = req.body;
        const db = await getDb();
        ...
        db.prepare('INSERT INTO completions (id, item_id, user_id, completed_at, scheduled_for, undone) VALUES (?, ?, ?, ?, ?, ?)').run(completionId, id, userId, completedAt, scheduledFor, 0);
        // Calculate next due date using scheduling service
...
router.post('/:id/undo', authenticateApiKey, validateBody(undoCompletionInputSchema), async (req, res) => {
    try {
        const { id } = req.params;
        const input = req.body;
        const db = await getDb();
        ...
        const nextDueAt = completion.scheduled_for || new Date().toISOString();
