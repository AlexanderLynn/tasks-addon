...
async function requireListAdmin(listId, userId) {
    ...
    const member = db.prepare('SELECT * FROM list_members WHERE list_id = ? AND user_id = ? AND permission = ?').get(listId, userId, 'admin');
    if (!member) {
...
router.get('/', authenticateApiKey, async (req, res) => {
    try {
        const db = await getDb();
        const lists = db.prepare('SELECT * FROM lists WHERE owner_id = ?').all(req.userId);
...
router.post('/', authenticateApiKey, validateBody(createListInputSchema), async (req, res) => {
    try {
        ...
        db.prepare('INSERT INTO lists (id, name, type, owner_id, created_at, updated_at, version) VALUES (?, ?, ?, ?, ?, ?, ?)').run(listId, input.name, input.type, ownerId, now, now, 0);
        // Add owner as admin member
        const memberId = uuidv4();
        db.prepare('INSERT INTO list_members (id, list_id, user_id, permission, joined_at) VALUES (?, ?, ?, ?, ?)').run(memberId, listId, ownerId, 'admin', now);
        // Log audit
        await logAudit({
            entityType: 'list',
            entityId: listId,
...
router.put('/:id', authenticateApiKey, validateBody(updateListInputSchema), async (req, res) => {
    try {
        const { id } = req.params;
        const input = req.body;
        const db = await getDb();
...
router.post('/:id/members', authenticateApiKey, validateBody(memberInputSchema), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const memberId = uuidv4();
        const now = new Date().toISOString();
        db.prepare('INSERT INTO list_members (id, list_id, user_id, permission, joined_at) VALUES (?, ?, ?, ?, ?)').run(memberId, id, userId, permission, now);
        await logAudit({
            entityType: 'member',
            entityId: memberId,
            action: 'create',
...
router.put('/:id/members/:userId', authenticateApiKey, validateBody(permissionInputSchema), async (req, res) => {
    try {
        const { id, userId } = req.params;
        const { permission } = req.body;
        const { db } = await requireListAdmin(id, req.userId);
        const member = db.prepare('SELECT * FROM list_members WHERE list_id = ? AND user_id = ?').get(id, userId);
        if (!member) {
...
router.delete('/:id/members/:userId', authenticateApiKey, async (req, res) => {
    try {
        const { id, userId } = req.params;
