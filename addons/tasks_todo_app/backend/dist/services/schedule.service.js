 ...
 */
export function calculateNextDueDate(schedule, lastCompleted) {
    const now = new Date();
...
function calculateDailyNext(schedule, baseDate, timezone) {
...
function calculateWeeklyNext(schedule, baseDate, timezone) {
    if (!schedule.daysOfWeek || schedule.daysOfWeek.length === 0) {
...
function calculateMonthlyNext(schedule, baseDate, timezone) {
    if (!schedule.dayOfMonth || schedule.dayOfMonth.length === 0) {
    ...
    const targetDays = schedule.dayOfMonth.sort((a, b) => a - b);
...
function calculateCustomNext(schedule, baseDate, timezone) {
...
function parseTime(time, date, timezone) {
