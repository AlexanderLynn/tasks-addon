...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const userId = await getUserIdFromApiKey(args?.apiKey);
    try {
        switch (name) {
            case 'list_items': {
                const { listId, type, status, assignedTo, limit = 50, offset = 0 } = args;
                ...
                const total = db.prepare('SELECT COUNT(*) as count FROM items WHERE status != ?').get('deleted');
                return {
            ...
            case 'create_item': {
                const { listId, title, description, type, schedule, assignedTo, tags, sharedWith } = args;
                const itemId = uuidv4();
                ...
                db.prepare(`INSERT INTO items (id, list_id, title, description, type, status, schedule, assigned_to, shared_with, tags, next_due_at, created_at, updated_at, version) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(itemId, listId, title, description || null, type, 'active', JSON.stringify(schedule), assignedTo || null, sharedWith ? JSON.stringify(sharedWith) : null, tags ? JSON.stringify(tags) : null, nextDueAt, now, now, 0);
                // Log audit
                await logAudit({
                    entityType: 'item',
                    entityId: itemId,
            ...
            case 'update_item': {
                const { id, title, description, type, status, schedule, assignedTo, tags, sharedWith, version } = args;
            ...
            case 'complete_item': {
                ...
                db.prepare('INSERT INTO completions (id, item_id, user_id, completed_at, scheduled_for, undone) VALUES (?, ?, ?, ?, ?, ?)').run(completionId, itemId, userId, completed, scheduledFor, 0);
            ...
            case 'undo_completion': {
                const { itemId, completionId } = args;
                let completion;
                if (completionId) {
            ...
            case 'list_completions': {
                const { itemId, userId: filterUserId, undone, limit = 50 } = args;
                ...
                const total = db.prepare('SELECT COUNT(*) as count FROM completions WHERE item_id = ?').get(itemId);
                return {
            ...
            case 'get_next_due': {
                ...
                const lastCompletedDate = lastCompletion && lastCompletion.completed_at ? new Date(lastCompletion.completed_at) : undefined;
            ...
            case 'search_items': {
                const { query, listId, limit = 50 } = args;
                const searchPattern = `%${query}%`;
                let sql = 'SELECT * FROM items WHERE status != ? AND (title LIKE ? OR description LIKE ? OR tags LIKE ?)';
                const params = ['deleted', searchPattern, searchPattern, searchPattern];
                if (listId) {
                    sql += ' AND list_id = ?';
                    params.push(listId);
                }
            ...
            case 'create_list': {
                ...
                const listId = uuidv4();
                const now = new Date().toISOString();
                db.prepare('INSERT INTO lists (id, name, type, owner_id, created_at, updated_at, version) VALUES (?, ?, ?, ?, ?, ?, ?)').run(listId, name, type, userId, now, now, 0);
                const memberId = uuidv4();
                db.prepare('INSERT INTO list_members (id, list_id, user_id, permission, joined_at) VALUES (?, ?, ?, ?, ?)').run(memberId, listId, userId, 'admin', now);
                // Log audit
                await logAudit({
                    entityType: 'list',
                    entityId: listId,
            ...
            case 'share_list': {
                const { listId, userId: targetUserId, permission } = args;
                const list = db.prepare('SELECT * FROM lists WHERE id = ?').get(listId);
                if (!list) {
                    return {
                ...
                const memberId = uuidv4();
                const now = new Date().toISOString();
                db.prepare('INSERT INTO list_members (id, list_id, user_id, permission, joined_at) VALUES (?, ?, ?, ?, ?)').run(memberId, listId, targetUserId, permission, now);
                // Log audit
                await logAudit({
                    entityType: 'member',
                    entityId: memberId,
            ...
            case 'remove_list_member': {
                const { listId, userId: targetUserId } = args;
                const list = db.prepare('SELECT * FROM lists WHERE id = ?').get(listId);
                if (!list) {
                    return {
                        content: [{ type: 'text', text: JSON.stringify({ error: 'List not found' }, null, 2) }],
